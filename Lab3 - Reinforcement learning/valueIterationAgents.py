...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for state in self.mdp.getStates():
            ...
            new_values[state] = valuesForActions[valuesForActions.argMax()]
            self.actions[state] = valuesForActions.argMax()
    ...
    def runValueIteration(self):
        ...
        for k in range(self.iterations):
            # a new dict to store the next values
            new_values = util.Counter()
            for state in self.mdp.getStates():
                valuesForActions = util.Counter()
                possibleActions = self.mdp.getPossibleActions(state)
                ...
                for action in possibleActions:
                    valuesForActions[action] = self.computeQValueFromValues(state, action)
                new_values[state] = valuesForActions[valuesForActions.argMax()]
                # update also actions dict
                self.actions[state] = valuesForActions.argMax()
            #save the new values in values
    ...
    def computeQValueFromValues(self, state, action):
        ...
        # calculate the sum for every next state
